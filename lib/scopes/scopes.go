...
// *must* be called on all scope values recieved from user input and/or cluster-external sources (e.g.
...
const (
	// Orhogonal indicates that the scopes are not related.
	Orthogonal Relationship = iota
	// Equivalent indicates that the scopes are equal.
	Equivalent
	// Ancestor indicates that one scope is an ancestor of another.
	Ancestor
	// Descendant indicates that one scope is a descendant of another.
	Descendant
)
